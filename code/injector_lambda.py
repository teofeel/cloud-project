...
TABLE_COLS = {
    ...
    "Top_Posts": ["post_id", "author_username", "content_text", "created_at","post_type", "score", "year", "month", "day", "rank", "platform", "date"],
    "Top_Users": ["username", "karma_score", "is_verified", "created_at","user_followers", "rank", "platform", "date"],
    "Bottom_Users": ["username", "karma_score", "is_verified", "created_at","user_followers", "rank", "platform", "date"],
...
def lambda_handler(event, context):
    try:
        ...
        try:
            conn.run('INSERT INTO public."Platform" (platform_id, platform_name) VALUES (:id, :p) ON CONFLICT (platform_id) DO NOTHING', id=platform_id, p=platform_name)
            conn.run('INSERT INTO public."Date" (date_id, "date") VALUES (:id, :d) ON CONFLICT (date_id) DO NOTHING', id=date_id, d=date_val)
